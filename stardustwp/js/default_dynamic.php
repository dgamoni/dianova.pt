...
if($qode_options_stardust['menu_lineheight'] != ""){
	$line_height = $qode_options_stardust['menu_lineheight'];
}else{
	$line_height = 80;
}
...
var line_height_a = <?php echo $line_height; ?> - 6;
var logo_height; // it's value is calculated in window load function
var height_span = <?php echo $line_height; ?> - 6;
var height_wpml = <?php echo $line_height; ?> - 5;

function headerSize(scroll){
	"use strict";
<?php
	if(isset($qode_options_stardust['fixed_menu_resize'])){
		if($qode_options_stardust['fixed_menu_resize'] === "yes"){
...
<?php
if($qode_options_stardust['enable_google_map'] != ""){
?>
...
if(isset($qode_options_stardust['google_maps_scroll_wheel'])){
	if ($qode_options_stardust['google_maps_scroll_wheel'] == "yes")
		$google_maps_scroll_wheel = true;
}
$google_maps_zoom = 12;
if(isset($qode_options_stardust['google_maps_zoom'])){
	if (intval($qode_options_stardust['google_maps_zoom']) > 0)
		$google_maps_zoom = intval($qode_options_stardust['google_maps_zoom']);
}
?>
	...
	geocoder.geocode( { 'address': data}, function(results, status) {
		if (status === google.maps.GeocoderStatus.OK) {
			...
			var marker = new google.maps.Marker({
				...
				<?php if(isset($qode_options_stardust['google_maps_pin_image'])){ ?>
				icon:  '<?php echo $qode_options_stardust['google_maps_pin_image']; ?>',
				<?php } ?>
...
function showContactMap() {
	...
	if($j("#map_canvas").length > 0){
		initialize();
		codeAddress('<?php if(isset($qode_options_stardust['google_maps_address'])) { echo $qode_options_stardust['google_maps_address']; } ?>');
	}
}

var no_ajax_pages = [];
var root = '<?php echo home_url(); ?>/';
<?php if($qode_options_stardust['parallax_speed'] != ''){ ?>
var parallax_speed = <?php echo $qode_options_stardust['parallax_speed']; ?>;
<?php }else{ ?>
...
if (isset($qode_options_stardust['internal_no_ajax_links'])) {
	foreach (explode(',', $qode_options_stardust['internal_no_ajax_links']) as $no_ajax_link) {
?>
